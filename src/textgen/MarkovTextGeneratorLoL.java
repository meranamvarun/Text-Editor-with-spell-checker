...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText)
	{
		...
		for(int i=1;i<words.length;i++){
			...
			if(wordList.isEmpty()){
				...
				wordList.get(0).addNextWord(words[i]);
			}
			
			else{
				for(ListNode ln:wordList){
					String word= ln.getWord();
				...
				if(success){
					...
					ListNode temp1= new ListNode(prevWord);
					wordList.add(temp1);
					wordList.get(wordList.indexOf(temp1)).addNextWord(words[i]);
				}
		...
		wordList.add(temp);
		wordList.get(wordList.indexOf(temp)).addNextWord(starter);
		...
		while(numWords > 1){
			...
			for(ListNode ln:wordList){
				if(currWord.equals(ln.getWord())){
					...
					currWord=ln.getRandomNextWord(rm);
	...
	public void retrain(String sourceText)
	{
		...
		for(int i=1;i<words.length;i++){
			...
			boolean success = true;
			if(wordList.isEmpty()){
				wordList.add(0, new ListNode(prevWord));
				wordList.get(0).addNextWord(words[i]);
			}
