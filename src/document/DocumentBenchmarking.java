	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			for(int i=0;i<trials;i++){
				EfficientDocument doc1 = new EfficientDocument(sample);
				doc1.getFleschScore();
				
			}
			long EndTimeEfficientDocument = System.nanoTime();	
			
			double  TotalTimeEfficientDocument = (EndTimeEfficientDocument - StartTimeEfficientDocument)/(double)1000000000;
			
			long StartTimeBasicDocument = System.nanoTime();
			for(int i=0;i<trials;i++){
			BasicDocument doc = new BasicDocument(sample);
			doc.getFleschScore();
			}
			long EndTimeBasicDocument = System.nanoTime();
			double TotaltimeBasicDocument=(EndTimeBasicDocument-StartTimeBasicDocument)/(double)1000000000;
			
			System.out.println(numToCheck+"\t"+TotaltimeBasicDocument+"\t"+TotalTimeEfficientDocument);
