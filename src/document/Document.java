...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(int i=0;i<charword.length;i++){
			...
			if(count>0 && i == charword.length-1 && startcount && Character.toLowerCase(charword[i])== 'e'){
				count--;
			}
			
			if(vowels.indexOf(Character.toLowerCase(charword[i]))>=0 &&startcount ){
				count++;
				startcount=false;
			}
			
			else if(vowels.indexOf(Character.toLowerCase(charword[i]))<0){
				startcount=true;
			}
	...
	public double getFleschScore()
	{
		...
		double fleshScore = (double) (206.835 - 1.015*((double)getNumWords()/getNumSentences())-84.6*((double)getNumSyllables()/getNumWords()));             
	    return fleshScore;
	}
